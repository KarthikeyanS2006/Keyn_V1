...
import { Download, Loader2, Image as ImageIcon, SendHorizontal, AlertCircle, Wand2, Sparkles, Clock, Trash2, Share2, Check, Copy, AlertTriangle, Palette } from 'lucide-react';
...
const Tooltip: React.FC<TooltipProps> = ({ text, children }) => {
  return (
    <div className="relative group">
      {children}
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block px-3 py-1.5 bg-zinc-800 dark:bg-zinc-100 text-white dark:text-zinc-900 text-xs font-medium rounded-lg shadow-xl whitespace-nowrap z-50 pointer-events-none transform transition-all duration-200">
        {text}
...
const ImageGenerator: React.FC = () => {
  ...
  const handlePromptChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    setIsPromptEnhanced(false); // Reset on manual edit
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Pass the selected style to the generation service
      const result = await generateImageContent(inputPrompt, aspectRatio, isPromptEnhanced, selectedStyle);
      
      // Handle Warnings (Graceful degradation)
      if (result.warning) {
      ...
      if (result.imageUrl) {
        ...
        const finalPrompt = result.textResponse || inputPrompt;
      ...
      if (err.code === 'NETWORK_ERROR') {
          ...
          errorMessage = "The AI could not generate an image for this prompt. Please try a different description.";
      }
  ...
  const restoreFromHistory = (item: HistoryItem) => {
    ...
    // Backward compatibility for items without style
  ...
  const getRatioTooltip = (ratio: AspectRatio) => {
    switch (ratio) {
      case AspectRatio.SQUARE: return 'Square (1:1) - Perfect for social media';
      case AspectRatio.PORTRAIT: return 'Portrait (9:16) - Full screen mobile';
      case AspectRatio.LANDSCAPE: return 'Landscape (16:9) - Desktop & Video';
      case AspectRatio.WIDE: return 'Wide (4:3) - Classic photography';
      case AspectRatio.TALL: return 'Tall (3:4) - Editorial composition';
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Input Section */}
      <div className="bg-white/80 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm transition-colors duration-300">
        <form onSubmit={handleGenerate} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-3 mb-2">
              <div className="flex items-center justify-between">
                <label htmlFor="prompt" className="text-sm font-medium text-zinc-600 dark:text-zinc-400 ml-1">
                  Describe your imagination
                </label>
                {isPromptEnhanced && (
                   <span className="text-xs text-orange-600 dark:text-orange-400 font-medium flex items-center gap-1 animate-in fade-in">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all whitespace-nowrap flex-shrink-0 border ${
                      selectedStyle === style
                        ...
                        : 'bg-white dark:bg-zinc-950 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 hover:border-orange-300 dark:hover:border-orange-500/50 hover:text-orange-600 dark:hover:text-orange-400'
                    }`}
                  >
                    {style}
                  </button>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
               {SAMPLE_PROMPTS.map((s, i) => (
                 <button
                   ...
                   className="text-[10px] sm:text-xs bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800/50 hover:border-orange-500/30 text-zinc-600 dark:text-zinc-500 hover:text-orange-600 dark:hover:text-orange-400 px-3 py-1.5 rounded-full transition-all hover:bg-zinc-100 dark:hover:bg-zinc-800 text-left"
                 >
                   {s}
                 </button>
               ))}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-2">
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <span className="text-sm text-zinc-500 whitespace-nowrap">Aspect Ratio:</span>
              <div className="flex bg-zinc-50 dark:bg-zinc-950 rounded-lg p-1 border border-zinc-200 dark:border-zinc-800 flex-wrap sm:flex-nowrap gap-1 w-full sm:w-auto">
                {Object.values(AspectRatio).map((ratio) => (
                  <Tooltip key={ratio} text={getRatioTooltip(ratio)}>
                    <button
                      type="button"
                      onClick={() => setAspectRatio(ratio)}
                      className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all whitespace-nowrap flex-grow sm:flex-grow-0 ${
                        aspectRatio === ratio
            ...
            <Tooltip text={isPromptEnhanced ? "Generate Image" : "Auto-Enhance & Generate"}>
              <button
                type="submit"
                disabled={loading || !prompt.trim()}
                className={`w-full sm:w-auto px-6 py-3 rounded-xl flex items-center justify-center gap-2 font-semibold transition-all shadow-lg shadow-orange-900/20 ${
                  loading || !prompt.trim()
      ...
      <div className="min-h-[400px] flex flex-col">
        {/* Error Alert */}
        {error && (
          <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl p-4 mb-6 flex items-center gap-3 text-red-600 dark:text-red-400 animate-in fade-in slide-in-from-top-2">
        ...
        {/* Warning Alert */}
        {warning && !error && (
          <div className="bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20 rounded-xl p-4 mb-6 flex items-center gap-3 text-yellow-700 dark:text-yellow-400 animate-in fade-in slide-in-from-top-2">
        ...
        {enhancedPrompt && (
          <div className="mb-4 p-4 bg-white/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl animate-in fade-in slide-in-from-bottom-2 duration-500 relative group/prompt transition-colors">
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover/prompt:opacity-100 transition-opacity">
            ...
            <p className="text-zinc-700 dark:text-zinc-300 text-sm italic pr-12 sm:pr-0">"{enhancedPrompt}"</p>
          </div>
        )}

        <div className={`relative flex-grow bg-zinc-50/50 dark:bg-zinc-900/30 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden flex items-center justify-center transition-all duration-500 ${generatedImage ? 'border-none bg-transparent' : ''}`}>
          
          {!generatedImage && !loading && (
            <div className="text-center p-8">
              <div className="w-20 h-20 bg-white dark:bg-zinc-900 rounded-full flex items-center justify-center mx-auto mb-4 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <ImageIcon className="w-8 h-8 text-zinc-400 dark:text-zinc-600" />
              </div>
              <h3 className="text-zinc-600 dark:text-zinc-400 font-medium mb-1">No image generated yet</h3>
              <p className="text-zinc-500 dark:text-zinc-600 text-sm max-w-xs mx-auto">
                Enter a descriptive prompt above and hit generate. AI will enhance your prompt for better results.
              </p>
            </div>
          )}
          ...
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/50 dark:bg-zinc-950/50 backdrop-blur-sm z-20">
              <div className="relative">
                <div className="w-16 h-16 border-4 border-zinc-200 dark:border-zinc-800 border-t-orange-500 rounded-full animate-spin"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <SparkleIcon />
                </div>
              </div>
              <p className="mt-4 text-orange-600 dark:text-orange-400 font-medium animate-pulse">Dreaming up your image...</p>
            </div>
          )}
          ...
          {generatedImage && (
            <div className="relative group w-full h-full flex items-center justify-center">
              ...
              <div className="absolute bottom-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <Tooltip text="Share image link">
                  <button
                    onClick={handleShare}
                    className="bg-white text-zinc-950 hover:bg-zinc-100 px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 shadow-lg transform transition-transform active:scale-95"
                  >
                ...
                <Tooltip text="Save image to device">
                  <button
      ...
      {history.length > 0 && (
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4 px-1">
             <h2 className="text-lg font-semibold text-zinc-800 dark:text-zinc-300 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {history.map((item) => (
              <div 
                ...
                className="group relative aspect-square bg-zinc-100 dark:bg-zinc-900 rounded-xl overflow-hidden cursor-pointer border border-zinc-200 dark:border-zinc-800 hover:border-orange-500/50 transition-all hover:shadow-lg hover:shadow-orange-900/10"
              >
                <img 
                  src={item.url} 
                  alt={item.prompt} 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-end p-3">
                  ...
                  <p className="text-[10px] text-zinc-300">
                    {item.aspectRatio} • {item.style || 'None'}
                  </p>
                </div>
              </div>
            ))}
